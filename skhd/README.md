...
- The default bindings assume Raycast-style Hyper (`cmd + alt + ctrl + shift`). Primary actions live on Hyper, `Hyper+Return` launches Ghostty, and swap/display/send/resize/warp use small modes entered with `Hyper+s`, `Hyper+d`, `Hyper+m`, `Hyper+e`, and `Hyper+w`.
